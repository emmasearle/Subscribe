...
/* BEGIN: Carousel lazy loader */
var CarouselLazyLoader = {};
CarouselLazyLoader.pageChanged = function (carousel, firstItemOnPageIndex) {
    // get the carousel items in a local variable
    var carouselItems = $(carousel.selector + " .horizontalSlideShow").children();
    // check that we found some carousel items
    if (carouselItems.length == 0) {
        // if we didn't, stop
        return;
    }
    // cycle through all the carousel items on the new page
    for (var i = firstItemOnPageIndex; i < carouselItems.length && i < firstItemOnPageIndex + carousel.dispItems; i++) {
        // check if the src attribute is already set
        if (!$(carouselItems[i]).find("img").attr("src")) {
            // it isn't so let's set it
            $(carouselItems[i]).find("img").attr("src", carousel.imageSrcArr[i]);
        }
    }
};
/* END: Carousel lazy loader */
